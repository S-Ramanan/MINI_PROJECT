...
void queue_wait_list(int bus_no,int trans[][2],struct wait_list psg[]);
...
int main(){
	printf("BUS TICKET BOOKING PROJECT");
	int i , j ;
	int a , bk , cll ;
	char c ;
...
void bus_chart()
{
	...
	printf("\n111         CHENNAI       MANIPUR          Rs.  293/- \n222         DELHI         MANIPUR              Rs.  209/- \n333         DELHI         KOVILPATTI             Rs.  312/- \n    ");
}
	...
	for(i=0;i<=rt;i++){
		for(j=0;j<30;j++){
			...
			for(m=0;m<wt;m++)
			{
				if(psg[m].pin==k)
				{
					printf("\n %s HAS BEEN ALLOTED SEAT NO : %d IN THE BUS NO %d",psg[m].name, (j+1),tr);
					wsub(1,tr);
					add(1,tr);
					a[j]='b';
					cValues(bus_no);
...
void booking(char fa[],int bus_no,int tr[][2],struct wait_list psg[],int trans[][2])
{
	...
	if( rcadd(bus_no) == 30 )
	{
		...
		if (c == 'Y' || c== 'y')
		{
			queue_wait_list(bus_no,trans,psg) ;
        }
...
//available seat for confirmed list
int space(int tr[][2])
{
	int i;
	for(i=0;i<30;i++)
...
//available seat for waiting list
int wspace(int tr[][2])
{
	int i;
	for(i=0;i<10;i++){
...
//rearrange the waiting list after allocation
void cValues(int a[][2])
{
	for(int j=0;j<10;j++)
	{
